...
def trprpathk(alpha,eta,path):
    ...
    for k in range(1,len(trpath)):
		# Assuming X(0) is a valid transaction price
        trpath[k]=updtrpr(alpha,eta[k-1],trpath[k-1],path[k])
        # trpath[k-1] was already changed from X to P
        # It's path-dependent (P(t) depends on P(t-1))
    # return trpath.transpose()[0]
    return trpath
...
# %% Asymptotic probabilities (t=1)

PROB_ASY_FLOOR = 1e-20

def four_ps(s0, α, η, σ, μ):
    pT_Al_Up = max(PROB_ASY_FLOOR, nprobw(+1, s0 + (+1) * (0.5 - η) * α, s0, α, η, σ, μ, 1))
    pT_Al_Down = max(PROB_ASY_FLOOR, nprobw(-1, s0 + (-1) * (0.5 - η) * α, s0, α, η, σ, μ, 1))
    p1_Al_Up = max(PROB_ASY_FLOOR, (pT_Al_Up * pT_Al_Down) / (pT_Al_Up + pT_Al_Down))
    p1_Co_Up = max(PROB_ASY_FLOOR, (pT_Al_Up * (1 - pT_Al_Down)) / (pT_Al_Up + pT_Al_Down))
    p1_Al_Down = max(PROB_ASY_FLOOR, (pT_Al_Up * pT_Al_Down) / (pT_Al_Up + pT_Al_Down))
    p1_Co_Down = max(PROB_ASY_FLOOR, ((1 - pT_Al_Up) * pT_Al_Down) / (pT_Al_Up + pT_Al_Down))
...
def multinom_likel(prior, hist):
    ...
    log3 = np.sum(np.array([np.log(prior[j]**hist[j]) for j in range(len(prior))]))
...
class Process():
    ...
    def __init__(self, ηs, σs, μs, roll_window=100, update_freq=10):
    ...
    def plot_marginals(self):   
        ...
        fig.suptitle('Marginals: ' + str(self.__count_roll[0]) +
    ...
    def calc_update_freq(self):
        ...
        for ηi in range(self.__ηn):
            ...
            for σi in range(self.__σn):
                ...
                for μi in range(self.__μn):
                    μ = self.__μs[μi]
                    # Use counts to update η, σ, μ
                    self.__likel_μ[ηi, σi, μi] = multinom_likel(four_ps(s, α, η, σ, μ), np.array(self.__count_roll[1]))
            ...
            for σi in range(self.__σn):
                ...
                for μi in range(self.__μn):
                    ...
                    self.__probs[ηi, σi, μi] = self.__prob_floor + self.__probs[ηi, σi, μi] *\
                        nprobmw(ud, -ud, self.__pxs.loc[self.__j - 1], αk, ηk, σ, μ, t)  # continuation: sud -> -ud
    ...
    def update_with_counts(self, trade):
        ...
        self.__Co_Down.loc[self.__j] = self.__Co_Down.loc[self.__j - 1]\
            + k * co * (1 - ud) // 2 + (k - 1) * al * (1 - ud) // 2
        self.__count = [self.__j,  # Make a rolling count as well, use it for better estimation
            ...
            for σi in range(self.__σn):
                ...
                for μi in range(self.__μn):
                    ...
                    if (self.__j % self.__uf) == 0:
                        self.__likel[ηi, σi, μi] = nprobmw(ud, sud, self.__pxs.loc[self.__j - 1], αk, ηk, σ, μ, t) *\
