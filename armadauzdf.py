...
# TRADING_HOURS = 9.25
# FILE1 = 'DOLG1720170127.csv'
# FILE2 = 'WDOG1720170127.csv'

# %% Constants

# DTOSEC = np.sqrt(TRADING_HOURS*3600)

# %% Functions applied to the initial data (selection, orders vs trades)

def column_datetime(data_frame):
...
def column_ot(data_frame):
    ...
    data_framec = data_frame.copy()
    data_framec['OT'] = data_framec['Trade Price'].isnull().copy()
    return data_framec

def select_times(data_frame, start_time, end_time):
    ...
    first_time = pd.to_datetime(data_framec['Date'].values[0]+start_time)
    mask_first = data_framec['DateTime'] >= first_time
    # Find last trade before end_time
    last_time = pd.to_datetime(data_framec['Date'].values[0]+end_time)
    ...
    del_index = data_framec[(data_framec['Ask 1 Price'] <=\
                             data_framec['Bid 1 Price'])].index
...
def collapse_time(data_frame):
    ...
    data_framecl.remove('Trade Qty')
    data_framec['DateTimem'] = data_framec['DateTime'] ==\
        data_frame['DateTime'].shift()
    data_framec['Trade Pricem'] = data_framec['Trade Price'] ==\
        data_frame['Trade Price'].shift()
    ...
    df_grouped = data_framec[['Trade Qty', 'nm']].groupby('nm').sum()
    ...
    return df_grouped_at_first

def collapse_price(data_frame):
    '''collapse_time(data_frame) takes a Data Frame of trades at various
...
def adduz(data_frame, alpha):
    ...
    data_framec = data_frame.copy()
    data_framec['dPj'] = data_framec['Trade Price'].diff()
...
def dur(spot, alpha, eta, vol):
    ...
    of the volatility'''
    return 2*eta*(alpha/(spot*vol))**2

def uz_stats(data_frame_trades, file_out, tick_value, trading_hours):
    '''uz_stats(data_frame, file_out, tick_value) returns the uncertainty
    zones statistics)'''
    df_columns = ['eta1', 'rvp', 'rvxe', 'dt_cont', 'dt_alt', 'dt_avg',\
                  'duration', 'chgavg', 'ndfpr', 'spot_avg']
    df_dict = dict(zip(df_columns, len(df_columns)*[0]))
    uz_df = uz_coal_byk(data_frame_trades)
    uz_df.set_index('Li', drop=False, inplace=True)
    #df_dict['eta1'] = uz_df['eta'].loc[1]
    df_dict['eta1'] = uz_df['eta'].values[0]
    df_dict['rvp'] = rlzvollog(data_frame_trades['Trade Price'])
    ...
    df_dict['spot_avg'] = data_frame_trades['Trade Price'].mean()
    df_dict['duration'] = dur(df_dict['spot_avg'], tick_value,\
